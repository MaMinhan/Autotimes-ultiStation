...
if __name__ == '__main__':  # 脚本入口（只有直接运行 run.py 才执行下面逻辑）
    ...
    parser.add_argument('--use_time', type=int, default=1,
                    help='whether to use time pt marks: 1 yes, 0 no')
    parser.add_argument('--use_weather', type=int, default=0,
                        help='whether to use weather pt marks: 1 yes, 0 no')
    # 你自定义 multi-station/时间embedding 使用的 time_only.pt 路径（全局时间轴embedding）
    parser.add_argument('--time_pt_path', type=str, default='')
    ...
    parser.add_argument('--train_pred_len', type=int, default=None,
                    help='train/val阶段的预测长度；默认等于token_len')
    ...
    parser.add_argument('--test_pred_len', type=int, default=24, help='test用的预测长度pred_len')
    ...
    parser.add_argument('--mark_input_dim', type=int, default=768,
                    help='mark输入维度：only-time=768, time+weather=1536')
    parser.add_argument('--weather_read_mode', type=str, default='torch')
    parser.add_argument('--weather_shard_dir', type=str, default='')
