...
def embed_texts_llama_original(
    ...
    for i in range(0, len(texts), batch_size):
        ...
        text_outputs = model.model(inputs_embeds=inputs_embeds)[0]  # [B, L, D]
        batch_embs = text_outputs[:, -1, :]  # [B, D]
...
def build_temp_window_text(
    ...
    return (
        # f"The temperature during this period has an average of {mean_temp:.1f} degrees Celsius, "
        # f"a minimum of {min_temp:.1f} degrees Celsius, and a maximum of {max_temp:.1f} degrees Celsius. "
        # f"It starts at {start_temp:.1f} degrees Celsius and ends at {end_temp:.1f} degrees Celsius. "
...
'''python /root/autotimes/embedding构建/weather/Llama_make_weather_pt_from_hourly_temperature_csv_eos_by_token_len.py \
  ...
  --out_weather_pt /root/autodl-tmp/datasets/SelfMadeAusgridData/weather/weather_token96_llama_time_and_temperature.pt \
  --token_len 96 \
  --batch_size 8 \
  --max_length 192 \
  --device cuda \
...
def main():
    ...
    # ------------------------------------------------------------
    # 4) Generate token-window embeddings
    # ------------------------------------------------------------
