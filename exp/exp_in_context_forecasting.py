...
class Exp_In_Context_Forecast(Exp_Basic):
    ...
    def train(self, setting):
        ...
        orig_root = self.args.root_path
        orig_path = self.args.data_path
        orig_data = self.args.data

        # ... 你改 args 做 test_loader2/3 ...

        self.args.root_path = orig_root
        self.args.data_path = orig_path
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                if i == 0:
                    print("outputs mean/std:", outputs.mean().item(), outputs.std().item())

                if i < 3 and ((not self.args.use_multi_gpu) or self.args.local_rank==0):
                    print("mark mean/std:", batch_x_mark.mean().item(), batch_x_mark.std().item())
